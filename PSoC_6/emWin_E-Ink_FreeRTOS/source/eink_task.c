...
extern GUI_CONST_STORAGE GUI_BITMAP bmpunisherLogo;
/*******************************************************************************
...
void show_startup_screen(void)
{
    ...
    GUI_DrawBitmap(&bmpunisherLogo, 2, 2);
...
void acceleroMeterPage(mtb_bmi160_data_t mtb_bmi160_data)
//{
	char temp[100];
    GUI_SetBkColor(GUI_WHITE);
    GUI_SetColor(GUI_BLACK);
    ...
    GUI_SetFont(GUI_FONT_24B_1);
    GUI_SetTextAlign(GUI_TA_HCENTER);
    GUI_DispStringAt("BMI MOTION", 132, 5);
    GUI_SetFont(GUI_TM_NORMAL);
    GUI_DispStringAt("Acccel : ", 5, 50);
    GUI_DispStringAt("Gyro : ", 137, 50);

    GUI_SetFont(GUI_FONT_24B_1);

#define SPACING		30
    sprintf(temp," X : %d",mtb_bmi160_data.accel.x);
    GUI_DispStringAt(temp, 5, 70);
    sprintf(temp," X : %d",mtb_bmi160_data.gyro.x);
    GUI_DispStringAt(temp, 137, 70);


    sprintf(temp," Y : %d",mtb_bmi160_data.accel.y);
    GUI_DispStringAt(temp, 5, 70+SPACING);
    sprintf(temp," Y : %d",mtb_bmi160_data.gyro.y);
    GUI_DispStringAt(temp, 137, 70+SPACING);


    sprintf(temp," Z : %d",mtb_bmi160_data.accel.z);
    GUI_DispStringAt(temp, 5, 70+SPACING*2);
    sprintf(temp," Z : %d",mtb_bmi160_data.gyro.z);
    GUI_DispStringAt(temp, 137, 70+SPACING*2);

//    sprintf(temp,"%d %d %d",mtb_bmi160_data.accel.x,mtb_bmi160_data.accel.y,mtb_bmi160_data.accel.z);
//    GUI_DispStringAt(temp, 5, 100);


    mtb_e2271cs021_show_frame(previous_frame, current_frame,
    		MTB_E2271CS021_PARTIAL, true);


}


uint8_t einkTaskReadyFlag = false ;
uint8_t geteinkTaskReadyFlag()
{
	return einkTaskReadyFlag;
}
...
//    wait_for_switch_press_and_release();
    ...
    mtb_bmi160_data_t mtb_bmi160_data;
    einkTaskReadyFlag = true;
    for(;;)
    {

    	xQueueReceive(motionDataQueue,&mtb_bmi160_data,portMAX_DELAY);
    	printf("\n[EINK TASK] : Update motion \r\n");

    	acceleroMeterPage(mtb_bmi160_data);
