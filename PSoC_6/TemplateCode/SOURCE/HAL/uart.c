...
void uart_5_InterruptInit(cy_cb_scb_uart_handle_events_t uart_5_Callback)
{
	...
	Cy_SCB_UART_RegisterCallback(CYBSP_DEBUG_UART_HW,uart_5_Callback,&uartContext);
}
...
void uart_5_Init(cy_cb_scb_uart_handle_events_t uart_5_Callback)
{
	...
	uart_5_InterruptInit(uart_5_Callback);
...
{
	Cy_SCB_UART_Transmit(SCB5, transmitData,strlen(transmitData), &uartContext);
}


uint32_t uart_5_receive()
{
	return Cy_SCB_ReadRxFifo(CYBSP_DEBUG_UART_HW);
}
