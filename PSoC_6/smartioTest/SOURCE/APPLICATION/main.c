...
//	void smartIOInit()
	...
	Cy_TCPWM_PWM_Init(PWM4_HW, PWM4_NUM,&PWM4_config);
    /* Enable the initialized PWM */
    Cy_TCPWM_PWM_Enable(PWM4_HW, PWM4_NUM);
    /* Then start the PWM */
    Cy_TCPWM_TriggerReloadOrIndex_Single(PWM4_HW, PWM4_MASK);
...
//Cy_GPIO_Write(TEST_OUT_1_PORT, TEST_OUT_1_PIN,0);
	...
	for (;;)
    {
		...
		Cy_GPIO_Write(CYBSP_USER_LED1_PORT, CYBSP_USER_LED1_PIN,Cy_GPIO_Read(SMART_IN1_PORT, SMART_IN3_PIN));
//		cyhal_gpio_write(CYBSP_USER_LED2,Cy_GPIO_Read(SMART_IN1_PORT, SMART_IN3_PIN));
		Cy_GPIO_Write(CYBSP_LED_RGB_BLUE_PORT, CYBSP_LED_RGB_BLUE_PIN,1);//Cy_GPIO_Read(SMART_IN1_PORT, SMART_IN1_PIN));
